    ...
    <div className="tab-page fade-in">
      <div className="page-header">
        ...
        <div className="status-card-main">
          ...
          <button className={`toggle-switch ${iaAtiva ? 'ia-active' : ''}`} onClick={() => { setIaAtiva(!iaAtiva); addLog(!iaAtiva ? 'IA Ativada' : 'IA Pausada', 'sistema'); }}>
      ...
      <div className="leads-sidebar">
        ...
        <div className="leads-list-clean">
          {leadsFiltrados.map(l => (
            <div key={l.id} className={`lead-card-clean ${ativo?.id === l.id ? 'ia-active' : ''}`} onClick={() => selectLead(l)}>
  ...
  return (
    <>
      ...
      <div className="gestao-container">
        <BTNVolta />
        <div className="ia-toast-container">{toasts.map(t => <Toast key={t.id} mensagem={t.mensagem} tipo={t.tipo} onClose={() => setToasts(prev => prev.filter(to => to.id !== t.id))} />)}</div>

        <div className="main-tabs-nav">
          <button className={`tab-btn ${activeTab === 'dashboard' ? 'ia-active' : ''}`} onClick={() => setActiveTab('dashboard')}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
            Dashboard
          </button>
          <button className={`tab-btn ${activeTab === 'leads' ? 'ia-active' : ''}`} onClick={() => setActiveTab('leads')}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
            Atendimento
            {contadores.pendentes > 0 && <span className="tab-badge">{contadores.pendentes}</span>}
          </button>
          <button className={`tab-btn ${activeTab === 'training' ? 'ia-active' : ''}`} onClick={() => setActiveTab('training')}>
          ...
          <button className={`tab-btn ${activeTab === 'logs' ? 'ia-active' : ''}`} onClick={() => setActiveTab('logs')}>
